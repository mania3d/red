...
function saveFlows(flows) {
    ...
    var defer = when.defer();
    collection().then(function(collection) {
        collection.updateOne({appname:appname},{$set:{appname:appname,flow:flows}},{upsert:true},function(err) {
            if (err) {
                defer.reject(err);
            } else {
                defer.resolve();
...
function saveSettings (settings) {
    var defer = when.defer();
    collection().then(function(collection) {
        collection.updateOne({appname:appname},{$set: {"settings":bconv(settings)}},{upsert:true},function(err) {
            if (err) {
                console.log(err);
                defer.reject(err);
            } else {
...
function saveFlow(fn,data) {
    var defer = when.defer();
    libCollection().then(function(libCollection) {
        libCollection.updateOne({appname:appname,type:'flow',path:fn},{appname:appname,type:'flow',path:fn,data:data},{upsert:true},function(err) {
            if (err) {
                defer.reject(err);
            } else {
                defer.resolve();
...
function saveLibraryEntry(type,path,meta,body) {
    var defer = when.defer();
    libCollection().then(function(libCollection) {
        libCollection.updateOne({appname:appname,type:type,path:path},{appname:appname,type:type,path:path,meta:meta,data:body},{upsert:true},function(err) {
            if (err) {
                defer.reject(err);
            } else {
                defer.resolve();
